...
import { encryptMessage, decryptMessage } from "../e2ee.js";
  ...
  async sendData(data) {
    ...
    if (!targetId) {
      ...
      this.ui.showNotification(
        "Cannot send data: No target selected.",
        "error"
      );
      return;
    }
    ...
    try {
      ...
      if (!this.state.getSharedSecret(targetId)) {
        console.warn(
          `No shared secret for ${targetId}. Attempting key exchange.`
        );
        ...
        if (!this.state.getSharedSecret(targetId)) {
          ...
          this.ui.showNotification(
            "Failed to send message: Security key missing.",
            "error"
          );
          return;
        }
      ...
      const encryptedData = await encryptMessage(
        JSON.stringify(data),
        targetId,
        this.state // Pass the StateManager instance
      );
      ...
      try {
        ...
        this.mediaRecorder.onstop = async () => {
          this.state
            .getLocalStream()
            .getTracks()
            .forEach((track) => track.stop()); // Stop stream tracks
  ...
  async handleIncomingMessage(data, senderId) {
    try {
      ...
      if (message.type === "typing") {
        ...
        if (fileData) {
          ...
          if (this.state.getCurrentTargetId() === senderId) {
            this.ui.renderMessages(
              this.state.getDiscussion(senderId).messages,
        ...
        const audioBlob = new Blob([base68ToUint8Array(message.data)], {
          type: "audio/webm",
        });
      ...
      } else if (message.type === "message-status" && !decrypted) {
        // Assuming status messages are not encrypted over signaling
        ...
        if (discussion) {
          ...
          if (this.state.getCurrentTargetId() === message.from) {
            this.ui.renderMessages(
              this.state.getDiscussion(message.from).messages,
              this.state.getClientId()
            );
          }
        }
      ...
      try {
        ...
        console.warn(
          "Message was encrypted but should not have been:",
          message
        );
        // Handle it as appropriate, or log a warning
      } catch (decryptionError) {
        console.error(
          "Failed to process message: Neither unencrypted nor encrypted with known key.",
          decryptionError
        );
      }
