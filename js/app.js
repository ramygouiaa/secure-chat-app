...
export class App {
    ...
    async start() {
        ...
        this.emitter.on("webrtc:datachannel-open", this.handleDataChannelOpen.bind(this));
        this.emitter.on("webrtc:datachannel-message", ({ data, fromId }) => this.dataManager.handleIncomingDataChannelMessage(data, fromId));
        this.emitter.on("webrtc:connection-state-change", ({ state, peerId }) => this.ui.updateConnectionStatus(`${state} ${peerId ? 'with ' + this.state.getPeerName(peerId) : ''}`));
        this.emitter.on("webrtc:ice-candidate", ({ candidate, peerId }) => this.signaling.sendIceCandidate(candidate, peerId));
        this.emitter.on("webrtc:offer-created", ({ offer, peerId }) => this.signaling.sendOffer(offer, peerId));
        this.emitter.on("webrtc:answer-created", ({ answer, peerId }) => this.signaling.sendAnswer(answer, peerId));
        ...
        this.emitter.on("webrtc:fallback-to-relay", this.handleFallbackToRelay.bind(this));
        this.emitter.on("webrtc:p2p-connected", ({ peerId }) => this.ui.updateConnectionStatus(`Connected (WebRTC) with ${this.state.getPeerName(peerId)}`));
        this.emitter.on("webrtc:disconnected", ({ peerId }) => {
            this.ui.updateConnectionStatus(`Disconnected from ${this.state.getPeerName(peerId)}`);
             // Consider attempting reconnect or falling back to relay here if not already active
        });

        this.emitter.on("data:message-updated", ({ peerId }) => { // DataManager tells App that messages for a peer have been updated
             if (peerId === this.state.getCurrentTargetId()) {
                 this.ui.renderMessages(this.state.getDiscussion(peerId).messages, this.state.getClientId());
             }
        });
        ...
        this.emitter.on("media:play-sound", ({ soundElement, loopCount }) => this.mediaManager.playAudioWithLoop(soundElement, loopCount));
        this.emitter.on("media:stop-sound", ({ soundElement }) => this.mediaManager.stopAudio(soundElement));
        this.emitter.on("media:set-mute-button-state", ({ isMuted }) => this.ui.setMuteButtonState(isMuted));

        // UI Events
        this.emitter.on("ui:enterChat", this.handleEnterChat.bind(this));
        this.emitter.on("ui:contactClick", ({ peerId, peerName }) => this.handleContactClick(peerId, peerName));
        this.emitter.on("ui:sendMessage", () => this.dataManager.sendMessage());
        this.emitter.on("ui:toggleRecording", () => this.dataManager.toggleRecording());
        this.emitter.on("ui:fileSelect", ({ event }) => this.dataManager.handleFileSelect(event));
        this.emitter.on("ui:initiateVoiceCall", () => this.mediaManager.initiateCall(false));
        this.emitter.on("ui:initiateVideoCall", () => this.mediaManager.initiateCall(true));
        ...
        this.emitter.on("ui:statusChange", ({ status }) => this.handleStatusChange(status));
        this.emitter.on("ui:forceRelayToggle", ({ isChecked }) => this.handleForceRelayToggle(isChecked));
        this.emitter.on("ui:contactSearch", ({ event }) => this.handleContactSearch(event));
    ...
    handlePeerList(peers) {
        ...
        this.ui.updateContactList(this.state.getAllPeers(), this.state.getClientId(), this.state.getCurrentTargetId(), this.state.getContactSearchTerm()); // Pass current target ID and search term
         // Ensure discussions are initialized for new peers
        peers.forEach(peer => {
            if (peer.id !== this.state.getClientId() && !this.state.getDiscussion(peer.id)) {
    ...
    handleForceRelayToggle(isChecked) {
         ...
         this.ui.showNotification(`Forced relay is now ${isChecked ? "ON" : "OFF"}. Reconnect to apply.`, "info");
     ...
     handleContactSearch(event) {
         ...
         this.ui.updateContactList(this.state.getAllPeers(), this.state.getClientId(), this.state.getCurrentTargetId(), searchTerm);
     }
     ...
     handleDataChannelOpen() {
        this.state.setIsRelayActive(false);
         ...
         if (this.state.getIceCandidateQueue(this.state.getCurrentTargetId())) {
             for (const candidate of this.state.getIceCandidateQueue(this.state.getCurrentTargetId())) {
          ...
          this.ui.renderMessages(this.state.getDiscussion(this.state.getCurrentTargetId()).messages, this.state.getClientId()); // Render messages after connection
     }
